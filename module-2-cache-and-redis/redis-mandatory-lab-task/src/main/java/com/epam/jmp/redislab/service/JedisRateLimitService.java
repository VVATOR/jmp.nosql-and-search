...
public class JedisRateLimitService implements RateLimitService {
    ...
    private static final int MILLIS_IN_SECOND_UNITS = 1_000;
    private static final int SECONDS_IN_MINUTE_UNITS = 60;
    private static final int MINUTES_IN_HOUR_UNITS = 60;
    private static final int SECONDS_IN_HOUR_UNITS = SECONDS_IN_MINUTE_UNITS * 60;
    ...
    public boolean shouldLimit(Set<RequestDescriptor> requestDescriptors) {
        long currentMinuteInUnix = System.currentTimeMillis() / MILLIS_IN_SECOND_UNITS / SECONDS_IN_MINUTE_UNITS;
        long currentHourInUnix = currentMinuteInUnix / MINUTES_IN_HOUR_UNITS;
        ...
        for (RequestDescriptor requestDescriptor : requestDescriptors) {
            ...
            if (appropriateRateLimitRule != null) {
                ...
                int expireSeconds = timeInterval == RateLimitTimeInterval.HOUR ? SECONDS_IN_HOUR_UNITS : SECONDS_IN_MINUTE_UNITS;
    ...
    private String getRateLimitRuleKey(RequestDescriptor requestDescriptor, RateLimitTimeInterval timeInterval, long unixTime) {
        ...
        if (isNotBlank(accountIdStr)) {
            rateLimitRuleKey.append(ACCOUNT_ID_KEY).append(accountIdStr).append(REDIS_KEY_DELIMITER);
        }
        if (isNotBlank(clientIpStr)) {
            rateLimitRuleKey.append(CLIENT_IP_KEY).append(clientIpStr).append(REDIS_KEY_DELIMITER);
        }
        if (isNotBlank(requestTypeStr)) {
            rateLimitRuleKey.append(REQUEST_TYPE_KEY).append(requestTypeStr).append(REDIS_KEY_DELIMITER);
        }
